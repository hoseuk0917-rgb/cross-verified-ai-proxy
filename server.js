...
// ✅ NEW: 전역 쿨다운만 갱신 (keyHint 등 "키ID가 없는 429"도 전역 차단하기 위함)
async function markGeminiKeyRateLimitedGlobal(userId, retryAfterMs) {
  if (!userId) return;
...
async function fetchGeminiRotating({ userId, keyHint, model, payload, opts = {} }) {
  ...
  let _rl429_count = 0;
  let _rl429_minRetryAfterMs = null;

  const _note429 = (raMs) => {
    _rl429_count += 1;
    const ms = Number.isFinite(raMs) ? raMs : 60000;
    _rl429_minRetryAfterMs = _rl429_minRetryAfterMs == null ? ms : Math.min(_rl429_minRetryAfterMs, ms);
  };
...
- 근거가 부족하거나 수치가 불확실하면, 숫자를 억지로 넣지 말고
  “~대/내외/수준/범위 가능” 같은 정성적 규모 + 필요한 기준(예: 총인구/주민등록인구/추계인구)을 명시하라.
...
- id는 1부터 순서대로.
- 숫자/통계 질문이라도 block.text에 숫자를 “삽입”하지 말라. 원문에 숫자가 있을 때만 그대로 포함하라.
